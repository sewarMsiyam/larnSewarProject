...
export default function HouerLib({ token }: CheckoutFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            toast.error(error.message || "حدث خطأ أثناء معالجة الموعد");
        } finally {
            setIsLoading(false);
            setIsOpen(false);
    ...
    const deleteOfficeHours = async (officeHoursId: number) => {
        setIsDeleting(true);
        try {
            const result = await deleteOneToken('instructor/instructor_office_hours', officeHoursId, token as string);
                                        ...
                                        <div className="flex justify-between items-center gap-3 border border-[#D9D9D9] font-bold p-3 rounded-xl px-6">
                                            <div className='flex flex-col gap-3'>
                                                ...
                                                <span className="flex items-center gap-2">
                                                    ...
                                                    من: {formatTimeTo12Hour(hour.from_time)} - إلى: {formatTimeTo12Hour(hour.to_time)}
                                                </span>
                                            </div>
                                            <div className='flex items-center gap-3'>
                                                <button onClick={() => handleUpdateClick(hour)} className="flex justify-center gap-2  bg-[#F1F1F1] p-3 rounded-xl w-full text-center">
                                                ...
                                                <button className="flex justify-center gap-2  bg-[#F1F1F1] p-3 rounded-xl w-full text-center"
